...
export default function Aurora(props: AuroraProps) {
  const {
    colorStops = ['#48F4AA', '#FCFCFC', '#F4B152'], // default per request
    amplitude = 1.0,
    blend = 0.5,
  } = props
  ...
  useEffect(() => {
    ...
    const gl = renderer.gl as unknown as OGLRenderingContext
    ...
    const geometry = new Triangle(gl as unknown as OGLRenderingContext)
    if ((geometry as any).attributes.uv) {
      delete (geometry as any).attributes.uv
    }

    const colorStopsArray = (colorStops || ['#48F4AA', '#FCFCFC', '#F4B152']).map((hex) => {
    ...
    const update = (t: number) => {
      ...
      const { speed = 1.0 } = propsRef.current
        ; (program!.uniforms.uTime as any).value = (t * 0.01) * speed * 0.1
      ...
      const stops = propsRef.current.colorStops ?? colorStops
        ; (program!.uniforms.uColorStops as any).value = (stops || ['#48F4AA', '#FCFCFC', '#F4B152']).map((hex) => {
          const c = new Color(hex)
          return [c.r, c.g, c.b]
        })
